...
const ASSASSIN_CROSS_SKILL_TSV_PATH = "src/data/skills-tsv/assassin-cross-skills.tsv";

type AssassinCrossSkillTsvRow = {
  id: string;
  job_id: string;
  job_tier: string;
  name: string;
...
type SkillTsvDirectory = {
  keyHeaders: string[];
  records: Array<AssassinCrossSkillTsvRow & { sourceRow: number }>;
};
...
function toTwoHeaderTsvRecords(text: string): SkillTsvDirectory {
  ...
  const records: Array<AssassinCrossSkillTsvRow & { sourceRow: number }> = [];
  ...
  for (let rowIndex = 2; rowIndex < rows.length; rowIndex += 1) {
    ...
    const record = { sourceRow: rowIndex - 2 } as AssassinCrossSkillTsvRow & { sourceRow: number };
    ...
    for (let columnIndex = 0; columnIndex < keyHeaders.length; columnIndex += 1) {
      ...
      record[key as keyof AssassinCrossSkillTsvRow] = (row[columnIndex] ??
        "") as AssassinCrossSkillTsvRow[keyof AssassinCrossSkillTsvRow];
    }
...
function mapJobTier(row: AssassinCrossSkillTsvRow): JobTier | null {
  const normalizedId = normalizeOptionalText(row.id);

  if (normalizedId.includes("_u2_")) {
    return "advancedSecond";
  }

  if (normalizedId.includes("_t2_")) {
    return "second";
  }

  if (normalizedId.includes("_t1_")) {
    return "first";
  }
...
function toSkillItem(row: AssassinCrossSkillTsvRow & { sourceRow: number }): SkillItem | null {
  const jobTier = mapJobTier(row);
...
const loadAssassinCrossSkillItems = cache(async (): Promise<SkillItem[] | null> => {
  try {
    const text = await readFile(ASSASSIN_CROSS_SKILL_TSV_PATH, "utf8");
    const { records } = toTwoHeaderTsvRecords(text);
    const items = records
      .filter((row) => row.job_id === "assassin" || row.job_id === "assassin_cross")
      .map(toSkillItem)
...
export async function getAssassinCrossSkillItemsFromTsv() {
  return loadAssassinCrossSkillItems();
}
